...
class UltraGCN(GeneralRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        interaction_matrix = dataset.inter_matrix(form='coo').astype(np.float64)
    ...
    def get_omegas(self, users, pos_items, neg_items):
        device = self.device
        users = torch.tensor(users).to(self.constraint_mat['beta_uD'].device)
        pos_items = torch.tensor(pos_items).to(self.constraint_mat['beta_iD'].device)
        neg_items = torch.tensor(neg_items).to(self.constraint_mat['beta_iD'].device)  # 确保 neg_items 在正确的设备上
        ...
        if self.w4 > 0:
            # 计算 beta_uD_repeated 的形状
            beta_uD_repeated = self.constraint_mat['beta_uD'][users].repeat_interleave(len(neg_items))
            beta_iD_neg = self.constraint_mat['beta_iD'][neg_items].repeat(len(users))
            neg_weight = torch.mul(beta_uD_repeated, beta_iD_neg).to(device)
            neg_weight = self.w3 + self.w4 * neg_weight
        else:
            neg_weight = self.w3 * torch.ones(len(users) * len(neg_items)).to(device)
    ...
    def cal_loss_I(self, users, pos_items):
        device = self.device
        pos_items = torch.tensor(pos_items).to(self.ii_neighbor_mat.device)  # 确保 pos_items 在正确的设备上
    ...
    def calculate_loss(self, interaction):
        ...
        # print(f"pos_items shape: {pos_items.shape}")
        # print(f"neg_items shape: {neg_items.shape}")

        omega_weight = self.get_omegas(users, pos_items, neg_items)

        # 调试输出
        # print(f"omega_weight length: {len(omega_weight)}")
